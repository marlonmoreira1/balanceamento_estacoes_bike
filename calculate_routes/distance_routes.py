...
def calculate_station_routes(df_routes: pd.DataFrame) -> Tuple[Dict, folium.Map]:
    ...
    n_donors = len(df_routes['nearby_station_id'].unique())
        
       
    routes_info = {}
    
   
    for idx, donor_name in enumerate(df_routes['name_nearby'].unique()):
        donor_routes = df_routes[df_routes['name_nearby'] == donor_name]               
        
        
        routes_info[donor_name] = {
            "start_point": {
                "nearby_name": donor_name,
                "coords": (donor_routes.iloc[0]['lat_nearby'], 
                          donor_routes.iloc[0]['lon_nearby'])
            },
            "destinations": []
        }
        ...
        for _, row in donor_routes.iterrows():
            ...
            if route:
                ...
                routes_info[donor_name]["destinations"].append({
                    "name": row['name'],
                    "coords": end_coords,
